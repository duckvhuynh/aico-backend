...
**Status:** Proposed semantic corrigendum for AICO-007 owner reacceptance; no proof-child result,
production implementation, R4 completion, or AT-014 pass is claimed
...
blocked until this corrected decision package is accepted on a new exact semantic SHA

## Withdrawn historical decision and pending reacceptance

- Historical semantic revision: `d30b76fb6aa47212450aee4cd592577f8df1300a`
- Architecture/Security owner: Duc Huynh (`@duckvhuynh`)
- Historical Architecture/Security decision:
  https://github.com/duckvhuynh/aico-backend/pull/22#issuecomment-5290436765
- Product/Platform owner: Duc Huynh (`@duckvhuynh`)
- Historical Product/Platform decision:
  https://github.com/duckvhuynh/aico-backend/pull/22#issuecomment-5290437423
- Historical candidate exact-SHA verification:
  https://github.com/duckvhuynh/aico-backend/actions/runs/31775182096
- Historical decision date: 2026-08-14
- Current Architecture/Security evidence: Pending
- Current Product/Platform evidence: Pending

Those comments accepted only the historical semantic revision. The browser capability and response
policy changed in a bounded semantic corrigendum, so neither historical comment accepts this
package. The old SHA and URLs are trace evidence only and MUST NOT authorize proof child #21. The
corrected package must pass Proposed-mode validation on a new clean 40-hex semantic SHA; separate,
fresh Architecture/Security and Product/Platform decisions must both cite that same SHA; only a
subsequent metadata-only revision may return the ADR to Accepted and pass accepted-mode hosted CI.

## 1. Decision outcome and evidence semantics

AICO-007 proposes a bounded preview-isolation architecture. The proposed contract permits only an
...
That statement is now **specified**, but not yet **selected**, **proved**, **implemented**, or
**release-qualified**:
...
- **Proved** means proof child #21 executes its closed deterministic fixture and mutation matrix on
  the reaccepted decision. It remains architecture-test evidence, not production evidence.
...
Later implementation cannot silently weaken the finally accepted AICO-007 boundary, while architecture
...
| ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ADR defines isolated origin/security headers, signed access, expiry/revocation, build integrity, caching, and cleanup.             | [`ADR-010`](../architecture/010-preview-isolation-selection.md), [`PREVIEW_ISOLATION.md`](../contracts/PREVIEW_ISOLATION.md), and [`preview-isolation.v1.schema.json`](../contracts/schemas/preview-isolation.v1.schema.json). | **Proposed corrigendum; owner acceptance pending.** Historical decisions do not apply to the new semantic SHA. | New exact-SHA Proposed validation, two fresh role-bound decisions, then metadata-only accepted-mode CI and #20 reconciliation.                           |
| Threat model covers control-plane request, cookie/storage access, navigation, script, another preview, and expired-token attempts. | [`AICO_007_THREAT_TEST_PLAN.md`](./AICO_007_THREAT_TEST_PLAN.md) defines the closed `A7-T-*` registry and `A7-M-*` mutation ownership.                                                                                         | **Proposed contract, not executed.** A registry is not a passing proof result.                                 | Reaccept the corrected decision; then #21 executes every required case/mutation without skip, waiver, survivor, prohibited evidence, or cleanup residue. |
| Spike serves fixture output and proves private control API access fails.                                                           | #21 defines the required spike outcome. No proof result bundle is part of this proposed decision package.                                                                                                                      | **Not met.** The proof child remains blocked and no fixture pass is claimed.                                   | #21 is unblocked only after corrected #20 reacceptance, then merges exact-SHA local/hosted evidence and criterion-level reconciliation.                  |
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `A7-ADR-01`       | ADR-010 compares delivery choices and proposes a reversible MVP boundary.                                                                                                                                                    | Proposed corrigendum; the historical SHA and decisions no longer accept the current semantics.                       | Proposed-mode clean-SHA CI, two fresh role-bound decisions on that SHA, then metadata-only accepted-mode CI and #20 reconciliation. |
...
| `A7-ACCESS-01`    | The server-side grant record binds tenant, preview, build/artifact/manifest versions and checksums, audience, expiry, epoch, nonce, key, and policy; the browser JWS exposes only the opaque digest-bound capability fields. | Contract is specified; no issuer, broker, or key custody exists here.                                                | #21 invalid/foreign/replay cases; production issuer/broker in AICO-057; release proof in AICO-083.                                  |
...
| `A7-TRACE-01`     | This file maps requirements, criteria, evidence state, gaps, owners, gates, and non-goals.                                                                                                                                   | Historical decisions, their semantic invalidation, and current gaps are explicit.                                    | Fresh acceptance, merge, and issue/Project reconciliation / #20 and Delivery.                                                       |
...
| `A7-VERIFY-01`    | Structural validator and document/schema mutation probes are included in the package.                                                                                                                                        | The corrected working tree has no clean semantic SHA or hosted verification yet.                                     | Proposed-mode clean-SHA CI, updated semantic mutations, then accepted-mode hosted CI after fresh decisions / #20.                   |
| `A7-ACCEPT-01`    | ADR reserves separate Architecture/Security and Product/Platform decisions.                                                                                                                                                  | Current Architecture/Security and Product/Platform evidence are both Pending. Historical URLs bind only the old SHA. | Fresh separate permanent role-bound decisions on the same new 40-hex semantic SHA, then accepted metadata merge / #20.              |
...
| Decision acceptance       | Proposed corrigendum; current evidence fields are Pending. Historical acceptance binds only the invalidated old semantic SHA.                                  | New clean semantic SHA, Proposed-mode hosted CI, two fresh permanent role-bound owner decisions on that SHA, then accepted-mode hosted CI and metadata-only #20 reconciliation.                   |
